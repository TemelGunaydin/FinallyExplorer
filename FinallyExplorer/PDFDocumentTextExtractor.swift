...
nonisolated struct PDFDocumentTextExtractor: Sendable {
    ...
    @concurrent func extract(_ data: Data, fileName: String, remainingOCRPages: Int,
        ...
        guard let pdf = PDFDocument(data: data) else { throw DocumentQuestionError.invalidPDF }
        guard pdf.isLocked == false else { throw DocumentQuestionError.passwordProtected }
        guard pdf.pageCount <= 100 else { throw DocumentQuestionError.pdfPageLimit(pdf.pageCount) }
        ...
        for index in 0..<pdf.pageCount {
            try Task.checkCancellation()
            guard let page = pdf.page(at: index) else { throw DocumentQuestionError.invalidPDF }
            guard page.numberOfCharacters <= 200_000 - characters else { throw DocumentQuestionError.textLimit }
            let text = page.string ?? ""
            characters += text.count
            guard characters <= 200_000 else { throw DocumentQuestionError.textLimit }
        ...
        for index in pages.indices where pages[index].isOCR {
            ...
            try Task.checkCancellation()
            guard let page = pdf.page(at: index)?.pageRef else { throw DocumentQuestionError.invalidPDF }
