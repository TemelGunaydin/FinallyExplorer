...
nonisolated struct FoundationModelsVisualInterpreter: VisualDescriptionInterpreting {
    ...
    @concurrent func interpret(_ query: String) async throws -> VisualDescriptionPlan {
        ...
        if let plan = try VisualDescriptionRouting.quickPlan(query) { return plan }
        guard query.range(of: #"\b(yesterday|today|tomorrow|days?|weeks?|months?|years?|before|after|without|except|exclude|not|delete|remove|move|copy|rename|larger|smaller|jpg|jpeg|png|heic)\b|\d"#,
                          options: [.regularExpression, .caseInsensitive]) == nil else { throw VisualDescriptionError.unsupportedRequest }
        try OnDeviceModelAccess.check(model, query: query)
        return try await OnDeviceModelAccess.bounded {
            let session = LanguageModelSession(model: model) {
                ...
                Sea/seaside photos -> [[beach, seashore, coast, coastal, ocean, sea]].
                Dogs on a beach -> [[dog, dogs, canine], [beach, seashore, coast]].
            ...
            return try VisualDescriptionPlan(concepts: output.concepts)
        }
    }
}
