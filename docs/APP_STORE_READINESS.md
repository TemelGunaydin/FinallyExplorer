...
- **Fresh/narrow window layout:** the QA app's normal first window was 900×450 points. Its preview covered some pane toolbar/path/search content; both the app-window screenshot and native hierarchy showed controls extending beyond the pane. Reproduce with synthetic-only content and fix minimum-size/responsive behavior before the final UI acceptance. This was observed, not fixed, in this test-only increment.
