...
### September 20 — Positive copy/move preparation

- Added post-copy/post-move byte preservation and pane-usability checks to the existing independent native case. Preflight of the unchanged installed sandboxed Release QA app passed; current user icon/project edits were preserved. Fresh `/private/tmp/fe-native-m20a` contains only the five known synthetic originals.
- The strengthened UI case compiles and **75 focused background tests passed, 0 failures/skips**, result `test_macos_2026-09-20T11-15-14-346Z_pid9811_98b13866.xcresult`. No foreground test was started; the independent native positive copy/move gate remains pending renewed user readiness. Production behavior/permissions and the earlier collision/ZIP results are unchanged. This is not a full background rerun or a release-readiness pass.

### September 20 — Independent positive copy/move acceptance passed

- The standalone native copy/move case passed in the unchanged, normally launched sandboxed Release QA app: **1 UI test, 0 failures/skips**, result `test_macos_2026-09-20T14-02-09-163Z_pid24229_a0d4e0a3.xcresult` (64.7 seconds MCP elapsed). Paste targets the displayed folder even with a file selected; repeated paste keeps the first copy and creates a renamed duplicate; cut/paste moves only that duplicate to the other pane.
- Both full pane paths, hittable rows and subsequent destination selection passed. Exact directory contents, original bytes and both generated copies' bytes were verified, including post-run hashes. Repeated-copy/final-move screenshots were inspected and teardown closed QA. Only the synthetic root was granted; production source, permissions and user icon/project edits remain unchanged.
- This completes the independent same-volume keyboard copy/move gate. Drag-and-drop, recursive folder copy and other outstanding file-operation coverage below remain separate. The preceding 75-test background result is retained, not counted as a new rerun; full background, ZIP and collision cases were not rerun in this documentation-only completion. Details are in [NATIVE_SANDBOX_ACCEPTANCE.md](NATIVE_SANDBOX_ACCEPTANCE.md).

## Remaining review items

- **Native access coverage:** upper name/plain-content/regex discovery, context-menu ZIP success/Unicode/hidden-file/name-collision/single-file behavior, independent same-volume keyboard copy/repeated-copy/move, and move-collision warning/dismissal with preserved files and usable panes through narrow grants now pass. Cross-pane drag-and-drop, recursive folder copy, explicit unselected-sibling denial, external volumes/reconnection, AI consumers and the remaining native file-operation matrix are still open.
