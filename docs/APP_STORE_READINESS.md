...
- **Native access coverage:** upper name/plain-content/regex discovery and context-menu ZIP success/Unicode/hidden-file/name-collision/single-file behavior through narrow grants now pass. Explicit unselected-sibling denial, external volumes/reconnection, AI consumers and the remaining native file-operation matrix are still open; move-collision alert accessibility/dismissal remains unresolved.
